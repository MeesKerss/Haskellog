...
Haskellog is an interpreter for a logic programming language similar to Prolog. The user gives a semantic meaning to some functions, and the program can then answer queries: it can determine whether a certain formula can be derived from the specified rules and, if it can, provide an instantiation of the variables that makes it derivable. If there are many such instantiations, Haskellog will provide up to 20 possibilities. Variables made be instantiated as constants but also as variables (for example if you define equality and query \verb|X=X|, you could get the answer \verb|X=X_12| which means that this is true for X being a variable which name here is X\_12.)

The program expects a file containing logical rules made from terms.
Terms are made with functions in lower case which can represent logical functions or logical predicates, variables in upper case, constants, which are simply functions of arity 0, and the logical connectors | (disjunction) and \& (conjunction). For the sake of readability, in the following examples we use integers as syntactic sugar for Peano arithmetic (1 is \verb|s(z)|, 2 is \verb|s(s(z))|...)
Each rule is either a fact, which is a single term like \verb|plus(X,z,X).| that you could interpret as $\forall X \in \mathbb{N}, X+0 = X$ or a rule composed of a conclusion and some premises, such as \verb|sum_of_consecutives(N) :- plus(A, 1, B) & plus(A,B,N).| which could be interpreted as "a number $N$ is a sum of two consecutive numbers $A$ and $B$ if $A + B = N$ and $A + 1 = B$". Obviously these interpretations are only given by the rules: the program does not know what \verb|plus| means to you and if you define it, it is your responsibility to provide rules that match your intended meaning of each predicate.
\newline
...
\subsection{Running Haskellog}

To build and run Haskellog run:
\begin{verbatim}
    stack build
    stack exec haskellog -- examples/family_relations.pl
\end{verbatim}
Multiple rule files can be passed at once; their clauses are concatenated into a single program.
Once the TUI opens, type a query in the input box and press \texttt{Enter}.
For example, with \texttt{family\_relations.pl} loaded, the query \verb|ancestor(george, X).| will return all descendants of George.
Press \texttt{Esc} or \texttt{Ctrl-C} to exit.
Type \verb|:help| to see additional built-in commands.
